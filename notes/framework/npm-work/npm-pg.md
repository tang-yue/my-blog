...
上述代码仅针对ios 手机，安卓手机不要执行上述获取旋转信息的代码了。

**踩坑理由：**目前已知在ios手机上会出现旋转，在安卓手机不会发生旋转==》判断旋转的`detectImageAutomaticRotation`函数只有ios手机的浏览器里才会返回`false`，而安卓手机的浏览器只会返回`true`。而现实是部分安卓手机的浏览器也会返回`false`。